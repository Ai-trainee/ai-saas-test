...
根据上面的分析，请严格按照如下格式输出，**直接输出示例开头的文字，接着输出 \`image_analysis\` 部分的内容，并使用示例中的 \`###\` 标题，最后输出 \`development_planning\` 部分的内容，并使用示例中的 \`###\` 标题，不要使用任何 HTML 标签包裹**：
...
const STRUCTURE_PROMPT = `请详细分析这个界面的板面结构，重点关注路由结构、页面实现和布局，并按照示例的这三个章节结构化输出提示词。

你需要考虑以下三个方面，但最终输出时请只包含路由结构、页面实现和布局的内容：

1.  **路由与导航**：
    -   应用的整体路由结构设计，**请使用示例中具体的路由名称，例如 \`/ai-tools\`, \`/ai-applications\` 等**。
    -   页面之间的导航关系，以及如何通过路由参数实现动态页面。
    -   路由的嵌套和层级结构。

2.  **页面布局**：
    -   页面的主要区块划分，例如头部、侧边栏、内容区域、页脚等。
    -   整体布局系统设计，例如使用网格系统、Flexbox 等技术进行布局。
    -   组件在页面中的复用策略，以及如何组合使用不同组件实现页面功能。
    -   响应式布局方案，如何针对不同设备和屏幕尺寸进行适配。

3.  **页面实现**:
    -   **每个页面的核心目的和主要功能**，并用精炼的语言描述。
    -   **页面中使用的关键组件**，以及它们的作用和相互关系。
    -   **页面内的数据流和状态管理**，如何加载数据、处理用户输入、更新UI。
    -   **请使用类似示例中的 “核心目的”、“关键组件”、“响应式行为” 等结构化描述**。

根据上面的分析，请严格按照如下格式输出，从路由结构开始（不是复制示例输出，而是按照示例格式输出，你需要参考具体图片），只输出 路由结构、页面实现、布局 三个章节的内容：
